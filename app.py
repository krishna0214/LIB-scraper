...
def process_with_params(supported_urls,api_key,format_option,wait_time):
    if supported_urls:
        with st.spinner("Processing...."):
            ...
            for idx, item in enumerate(scraped_data['data']):
                ...
                urls = extract_urls_from_html(raw_html)
...
def main():
    ...
    # Sidebar inputs
    Domain = st.sidebar.selectbox("Distributor:", options=["iewc"], index=0,on_change=param_changed)
    ...
    if st.sidebar.button("Submit"):
        ...
        finalUrls,metadata=process_with_params(st.session_state.supported_urls, api_key, format_option, wait_time)
